...
public:
	// Actuators:
	double front,dump; // linear actuators, 0-1 range
...
/* Coordinate systems */
	...
	*/
	...
	enum {wheelbase=62}; // left-right centimeters from centerline to wheel drive point (X)
...
/* Simulate these robot power values, for this timestep (seconds) */
	...
	// Update linear actuators
		double linear_scale=1.0/17.0/64.0; // seconds to full deploy, and power scale factor
		front+=-dt*(power.front-64.0)*linear_scale;
		if (front<0.0) front=0.0;
		if (front>1.0) front=1.0;
		
		dump+=dt*(power.dump-64.0)*linear_scale;
		if (dump<0.0) dump=0.0; 
		if (dump>1.0) dump=1.0;
	}
};
