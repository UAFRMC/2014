...
void robot_serial::update(robot_current &robot){
	...
	do {
		...
		if (p.valid) {
			if (p.command==0)
			...
			else if (p.command==0x3) { // sensor data
			...
			else if (p.command==0xB) { // blinky data
				...
				*/
				...
				robotPrintln("Got unknown packet type 0x%x length %d from robot",
					p.command,p.length);
			}
		}
		else { /* some sort of serial error? */
			break;
		}
